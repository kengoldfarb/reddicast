...
const useDuplicates = ({ enabled, permalink }) => {
	...
	const fetchDuplicates = async (fetchParams) => {
		...
		const res = await findDuplicates(context.token, feedParams.loggedIn, permalink, feedParams.after, feedParams.count)
		const data = res?.res?.[1]?.data
		const totalDuplicates = res?.res?.[0]?.data?.children?.[0]?.data?.num_duplicates
	...
	const duplicateQuery = useInfiniteQuery(['duplicates', permalink], fetchDuplicates, {
