...
const useFilterSubs = () => {
	...
	const addSubFilter = async (sub: string, showToast = true) => {
		...
		if (exists === undefined && sub.length > 0) {
			...
			showToast &&
				toast.custom((t) => <ToastCustom t={t} message={`Added r/${sub} to filters`} mode={'success'} />, {
			...
			showToast &&
				toast.custom((t) => <ToastCustom t={t} message={`r/${sub} already filtered`} mode={'error'} />, {
					position: 'bottom-center',
					duration: 3000
				})
		}
	...
	const addUserFilter = async (user: string, showToast = true) => {
		...
		if (exists === undefined && user.length > 0) {
			...
			showToast &&
				toast.custom((t) => <ToastCustom t={t} message={`Added u/${user} to filters`} mode={'success'} />, {
					position: 'bottom-center',
					duration: 3000
				})
			setUpdateFilters((n) => n + 1)
			...
			showToast &&
				toast.custom((t) => <ToastCustom t={t} message={`u/${user} already filtered`} mode={'error'} />, {
					position: 'bottom-center',
					duration: 3000
				})
		}
