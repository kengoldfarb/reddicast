...
const useMutate = () => {
	...
	const optimisticUpdate = async (key: string[], id: string, change: Change) => {
	...
	const saveMutation = useMutation(({ id, isSaved }: any) => saveLink('', id, isSaved), {
		onMutate: async (update) => {
			if (update.id.substring(0, 3) === 't3_') {
				return optimisticUpdate(['feed'], update.id, {
					property: 'saved',
	...
	const hideMutation = useMutation(({ id, isHidden }: any) => hideLink(id, isHidden), {
		onMutate: async (update) => {
			return optimisticUpdate(['feed'], update.id, {
				property: 'hidden',
	...
	const postCommentMutation = useMutation(({ parent, textValue, postName }: any) => postComment(parent, textValue), {
		onSuccess: (data: any) => {
			if (session?.user?.name) {
				queryClient.invalidateQueries(['feed', 'SELF', session.user.name, 'comments'])
			}
			...
			if (data?.parent_id?.substring?.(0, 3) === 't3_') {
				...
				queryClient.setQueriesData(['thread', data?.link_id?.substring?.(3)], (pCommentsData: any) => {
									...
									return {
										...
										data: {
											...
											replies: {
												...
												data: {
													...comment.data.replies.data,
													children: comment?.data?.replies?.data?.children?.map((cComment) =>
														editNestedComment(cComment, data, data.parent_id)
													)
												}
											}
										}
	...
	const editCommentMutation = useMutation(
		({ parent, text }: { parent: string; text: string }) => editUserText(parent, text),
		{
			onSuccess: (data) => {
				//console.log("success:", data, data.permalink.split("/")?.[4]);
				queryClient.setQueriesData(['thread', data.permalink.split('/')?.[4]], (pCommentsData: any) =>
	...
	const commentCollapse = useMutation(({ name, thread, collapse }: any) => dummy(), {
		onMutate: async (update) => {
			queryClient.setQueriesData(['thread', update.thread], (pCommentsData: any) =>
				updateCommentProperties(pCommentsData, update.name, [{ property: 'collapsed', value: update.collapse }])
			)
		}
	})

	const commentDelete = useMutation(({ name, thread }: { name: string; thread: string }) => deleteLink(name), {
		onMutate: async (update) => {
			queryClient.setQueriesData(['thread', update.thread], (pCommentsData: any) =>
				updateCommentProperties(pCommentsData, update.name, [{ property: 'deleted', value: true }])
			)
		}
	})

	const loadChildComments = async (parentName, children: any[], link_id, permalink, childcomments, token?) => {
	...
	const loadCommentsMutation = useMutation(
		({ parentName, children, link_id, permalink, childcomments, token }: any) =>
			loadChildComments(parentName, children, link_id, permalink, childcomments, token),
		{
			onSuccess: (data) => {
				queryClient.setQueriesData(['thread', data?.link_id?.substring(3)], (pCommentsData: any) =>
					updateCommentProperties(pCommentsData, data.parentName, [{ property: '', value: data.newComments }], true)
				)
			}
		}
	)
