...
const FilterSubs = ({ mode = 'subs' }) => {
	const { filteredSubs, filteredUsers, addSubFilter, removeSubFilter, addUserFilter, removeUserFilter } =
		useFilterSubs()
	...
	return (
		<div className='flex flex-col gap-2 border rounded-md bg-th-background2 border-th-border'>
			...
			{openForm ? (
				<form onSubmit={(e) => handleSubmit(e)} className='flex items-center flex-grow mx-2 mb-2 mr-4 rounded-md '>
					<input
						ref={inputRef}
						type='text'
						placeholder='Add..'
