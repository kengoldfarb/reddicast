	...
	useEffect(() => {
		if (expand) {
			if (router?.query?.type === 'sr' && searchUsers) {
				router.replace(router.asPath.replace(`&type=${router.query.type}`, '&type=user'))
			} else if (router?.query?.type === 'user' && !searchUsers) {
				router.replace(router.asPath.replace(`&type=${router.query.type}`, '&type=sr'))
			}
		}
			...
			<div className='flex flex-col items-center flex-none w-screen '>
				...
				>
					...
					>
						...
						<div
							className={
								expand ? ' mb-10 mt-2 flex flex-col gap-3  md:w-[32rem] lg:w-[48rem] xl:w-[54rem] 2xl:w-[60rem] ' : ''
							}
						>
							{loading && (
								<div
															...
															if (!expand) {
																router.push(
																	router.query?.type
																		...
																		: router.asPath.replace('search?', `search?type=${searchUsers ? 'user' : 'sr'}&`)
																)
															} else if (after) {
																loadMore()
															}
								...
								!loading && (
									<>
										<div className={'relative mb-2' + (context.cardStyle === 'row1' ? ' ' : ' mx-1 ')}>
											...
											>
												<h1>{`Didn't find any ${searchUsers ? 'users' : 'subreddits'} for "${query.q}"`}</h1>
