...
const ToggleFilters = ({ filter, withSubtext = false, quickToggle = false }) => {
	...
	useEffect(() => {
		...
		switch (filter) {
			...
			case 'images':
				...
				break
			case 'videos':
				setSubtext('Filter or show videos (or gifs). Only applies to native videos.')
				f = 'vidFilter'
				break
			case 'galleries':
	...
	return (
		<div title={title} onClick={(e) => e.stopPropagation()} className='rounded-lg group hover:bg-th-highlight'>
			<label className='flex flex-row items-center justify-between p-2 cursor-pointer '>
				<span className='flex flex-col'>
					<span className='capitalize '>{filter}</span>
					{withSubtext && <span className='mr-2 text-xs opacity-70'>{subtext}</span>}
				</span>
				<ReactSwitch
					onChange={() => {
						setToggled(true)
