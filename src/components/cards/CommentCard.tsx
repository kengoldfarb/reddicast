				...
				<div className='flex flex-row justify-start'>
					...
					<div className='flex flex-col'>
						<div className='flex flex-row flex-wrap space-x-1'>
							...
							>
								<h2 className='pl-1 hover:underline'>u/{data?.author ?? ''}</h2>
							</Link>
							<span className='text-th-textLight'>commented on</span>
							...
							>
								<h2 className=' hover:underline'>{data?.subreddit_name_prefixed ?? ''}</h2>
							</Link>
							<p className='text-th-textLight' title={new Date(data?.created_utc * 1000)?.toString()}>
								{secondsToTime(data?.created_utc, ['s ago', 'min ago', 'hr ago', 'dy ago', 'mo ago', 'yr ago'])}
							</p>
						</div>
					</div>
				</div>
