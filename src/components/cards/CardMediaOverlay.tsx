...
const CardMediaOverlay = ({ post, voteScore, setShowCardMediaOverlay }) => {
	...
	const mediaHeight = Math.min(
		windowHeight * 0.8,
		((windowWidth * 0.9) / post?.mediaInfo?.dimensions?.[0]) * post?.mediaInfo?.dimensions?.[1]
	)
	...
	return (
		...
		>
			...
			>
				...
				>
					<Media post={post} postMode={true} columns={1} containerDims={[windowWidth * 0.9, mediaHeight]} />
				</div>
