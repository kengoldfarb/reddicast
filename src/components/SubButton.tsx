...
const SubButton = ({ sub, miniMode = false, userMode = false }) => {
	...
	const { mySubs, myFollowing, myLocalSubs, myMultis, subscribe, loadedSubs, addToSubCache } = subsContext
					...
					{subbed && !loadAPI ? (
						<div className='flex items-center space-x-1 group'>
							{miniMode ? (
								...
								<>
									<span className={hovered ? 'hidden' : ''}>{userMode ? 'Followed' : 'Joined'}</span>
									<span className={hovered ? 'block' : 'hidden'}>{userMode ? 'Unfollow' : 'Leave'}</span>
								</>
							)}
						</div>
					) : !subbed && !loadAPI ? (
						<div className='flex items-center space-x-1'>
							{miniMode ? <AiOutlinePlus /> : <span>{userMode ? 'Follow' : 'Join'}</span>}
						</div>
					) : loadAPI ? (
						<div className={!miniMode ? 'p-2' : ''}>
							<ImSpinner2 className='animate-spin' />
