	...
	const favoriteSubs = useMemo(() => {
		if (session?.user?.name) {
			return [
				...
				...myFollowing?.filter((user) => user?.data?.subreddit?.user_has_favorited)
			]
		} else {
													...
													<MyLink href={sub?.data?.subreddit?.url?.replace('/user/', '/u/') ?? sub?.data?.url}>
														...
														>
															<DropdownItem
																...
																isUser={sub?.kind === 't2' || sub?.data?.display_name?.substring(0, 2) === 'u_'}
															/>
														</div>
													</MyLink>
												)}
										...
										? myMultis.map((multi, i) => {
												return (
													<Menu.Item
														...
														disabled={
															!expandMultis ||
															(filter !== '' && !multi.data.display_name.toUpperCase().includes(filter.toUpperCase()))
														}
													>
														{({ active }) => (
															<MyLink href={constructMultiLink(multi)}>
