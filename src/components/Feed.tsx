...
const Feed = ({ initialData = {} as any }) => {
	...
	const { invalidateAll, invalidateKey, refreshCurrent, fetchingCount } = useRefresh()
	...
	if (feed.error && (mode === 'USER' || mode === 'SUBREDDIT') && subreddits && !feed?.data?.pages) {
		toast.custom(
			(t) => (
				<ToastCustom
					t={t}
					message={`We couldn't find that ${mode === 'SUBREDDIT' ? 'subreddit' : 'user'}. ${
...
function ErrorFallback({ error, resetErrorBoundary }) {
	return (
		<div className='flex flex-col items-center justify-center mb-auto' role='alert'>
