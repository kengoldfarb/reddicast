...
const FeedMasonry = ({ initItems, feed, curKey }: MyMasonicProps) => {
	...
	const colWidth =
		context.cardStyle === 'row1'
			? width
			: (width - (cols ?? 0 > 1 ? gutter * (cols ? cols : 1) : 0)) / (cols ? cols : 1)
	...
	useEffect(() => {
		if (flatPosts.length > 0 && curKey.length > 1 && !scrolled) {
			...
			if (g) {
				...
				const lastPostIndex = flatPosts.findIndex((p) => lastPost && p?.data?.name === lastPost)
	...
	const { setHeight, getHeights } = useHeightMap({
		columnWidth: context.cardStyle === 'row1' ? width : Math.round(width / ((cols ? cols : 1) + 1)),
	...
	const handleIntersectChange = useCallback((inView: boolean, entry: IntersectionObserverEntry, post) => {
		if (
	...
	const RenderCard = useCallback(
		(props) => {
			...
			if ((context.cardStyle === 'card1' || context.cardStyle === 'default') && !context.mediaOnly) {
			...
			const linkMode =
				...
				!(post?.mediaInfo?.isIframe && (context.embedsEverywhere || (cols === 1 && !context.disableEmbeds)))
			const maxHeight = cols === 1 ? windowHeightToUse * 0.75 : windowHeightToUse * 0.9
			let mediaHeight = Math.min(
				post?.mediaInfo?.dimensions?.[1]
					? (mediaWidth / post?.mediaInfo?.dimensions?.[0]) * post?.mediaInfo?.dimensions?.[1]
					: 0,
				maxHeight
			)
			...
			if (post?.data?.mediaInfo?.isGallery) {
				...
				if (cols === 1) {
					mediaHeight = Math.min(windowHeightToUse * 0.75, ratio?.height * (width / ratio?.width))
				} else {
			...
			if (uniformMediaMode || (post.mediaInfo.isSelf && !post.mediaInfo.isDual)) {
				mediaHeight = 0
			}
			...
			return (
				<InView
					role={'gridcell'}
					threshold={0}
					onChange={(inView, entry) => handleIntersectChange(inView, entry, post)}
				>
					{({ inView, ref }) => (
						<div
							ref={ref}
	...
	const feedLoading = feed.isFetching && !feed.isError && context.infiniteLoading

	const loadInfo = (
		<>
			{!feed.isFetching && !feed.hasNextPage && feed.isFetched && !feed.error && flatPosts?.length > 0 && (
				<div className='flex flex-row items-center justify-center my-6 text-sm font-light'>
					<p>
						Loaded {flatPosts?.length} post
						{flatPosts?.length === 1 ? '' : 's'} on {feed.data?.pages?.length} page
		...
		<>
			{selectedPost && (
				<PostModal
					...
					returnRoute={router.query?.slug?.[1]?.toUpperCase() === 'M' ? 'multimode' : selectedPost.lastRoute}
