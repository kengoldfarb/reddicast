...
const SubOptButton = ({ subInfo, subArray, currMulti, openDescription = () => {} }) => {
	...
	useEffect(() => {
		const findIfMultiExist = () => {
			...
			if (session && myMultis?.length > 0) {
				myMultis.forEach((m) => {
					if (m?.data?.display_name?.toUpperCase() === currMulti.toUpperCase()) found = true
				})
			...
			myMultis.forEach((m) => {
				if (m?.data?.display_name?.toUpperCase() === currMulti.toUpperCase())
					m?.data?.subreddits.forEach((s) => {
						if (s?.name?.toUpperCase() === subInfo?.display_name?.toUpperCase()) found = true
					})
			})
		} else if (myLocalMultis?.length > 0) {
			myLocalMultis.forEach((m) => {
	...
	const removeFromMulti = () => {
		...
		if (session) {
			removeFromRedditMulti(currMulti, session?.user?.name, subInfo?.display_name)
		} else if (!loading) {
	...
	return (
		<>
			<MultiManageModal toOpen={openMulti} subreddits={subArray} multiname={currMulti} mode={'create'} />
										...
										{({ active }) => (
											<div className={(active ? 'bg-th-highlight ' : '') + ' block  text-sm'}>
												...
												>
													...
													>
														<div className='px-2 py-1 hover:bg-th-highlight hover:cursor-pointer' onClick={multiCreate}>
												...
												<div className={(active ? 'bg-th-highlight ' : '') + ' block px-4 py-2.5 md:py-1 text-sm'}>
													...
													>
														{deleteCheck ? (
															<div>
																{'Delete? '}
																<span className='hover:font-bold hover:underline' onClick={deleteMulti}>
																	Yes
																</span>{' '}
																/ <span className='hover:underline hover:font-bold'>No</span>
															</div>
														) : (
															<div
																onClick={(e) => {
