...
const VideoHandler = ({
	...
	const [videoQuality, setVideoQuality] = useState<'full' | 'sd' | 'hd' | 'min' | undefined>(() => quality)
	useEffect(() => {
	...
	const [muted, setMuted] = useState(!(context?.audioOnHover && postMode) || preventPlay)
	...
	useEffect(() => {
		...
		//if container (containerDims) fill it
			...
			if (Math.abs(ry - rx) < 0.05) {
				...
				setVidWidth(containerDims?.[0])
				setVidHeight(videoInfo.height * (containerDims?.[0] / videoInfo?.width))
			}
		}
		//
	...
	}, [imgFull, maxHeightNum, fullWidthRef?.current?.clientWidth, videoInfo, postMode, containerDims, columns])
	...
	}, [context.autoplay, context.pauseAll, context?.autoPlayMode, show, preventPlay, curPostName])
	...
	const playControl = ({
		...
		if (override === 'play') {
			...
			if ((videoRef?.current?.paused || videoRef?.current?.ended) && !videoPlaying) {
				//play the video
				playMedia()
			}
			//pause the video
	...
	const handleMouseIn = (e) => {
		...
		if (context.hoverplay && context.cardStyle !== 'row1') {
			if ((!manualPlay || videoRef?.current?.paused) && !context.autoplay && !postMode) {
				playControl({ e: e, override: 'play' })
			}
		}
		if (!postMode && !manualAudio && context.audioOnHover && !preventPlay && videoRef.current) {
	...
	const handleMouseOut = () => {
		...
		if (!manualAudio && !postMode && context.audioOnHover && columns !== 1 && videoRef.current) {
			videoRef.current.muted = true
			setMuted(true)
		}
	}
	...
	const updateSeek = (e) => {
		...
		if (videoRef.current) {
			...
			if (!videoPlaying) {
				setProgressPerc(videoRef?.current?.currentTime / videoRef?.current?.duration)
			}
	...
	const updateVolume = (e) => {
		e.stopPropagation()
		e.preventDefault()
		let r = Math.abs(1 - e.nativeEvent.offsetY / e.nativeEvent.target.clientHeight)
		if (r >= 1) r = 1
		if (r <= 0.1) r = 0
	...
	useEffect(() => {
		if ((!show || !context?.audioOnHover || left) && (!postMode || fullMediaMode) && videoRef.current) {
			videoRef.current.muted = true
			setMuted(true)
		} else if (
		...
		if (videoPlaying && videoRef?.current && (mouseIn || postMode || columns === 1) && !preventPlay) {
			...
			const updateSeekRange = () => {
				if (videoPlaying) {
					...
					//handle out of range..
					if ((initialTime + timepassed) / 1000 > duration && videoRef.current && videoRef?.current?.currentTime >= 0) {
	...
	useEffect(() => {
		if (progressPerc >= 0.9 && fullMediaMode && !setData.current && !preventPlay) {
	...
	return (
        ...
        {vidWidth},{vidHeight}
			...
			<div className='absolute z-0 min-w-full min-h-full overflow-hidden brightness-[0.2]' ref={fullWidthRef}>
				<Image
						...
						<div className={'flex items-center gap-2'}>
							...
							>
								<div className=''>
									{videoPlaying ? (
										<>
											{windowWidth <= 640 && (columns === 1 || postMode) && !fullMediaMode ? (
												<div className={' text-xs '}>{secondsToHMS(videoDuration - currentTime)}</div>
											) : (
