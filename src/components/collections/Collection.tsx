...
import { BsArrowRight, BsBoxArrowInUpRight, BsChevronDown } from 'react-icons/bs'
const CheckBox = ({ toggled }) => {
	return (
		<div
			className={`flex h-full border rounded-md transition-all items-center justify-center  ${
				toggled ? '  bg-th-accent border-th-accent ' : ' group-hover:bg-th-highlight border-th-border  '
			}`}
		>
			<AiOutlineCheck className={` transition-all text-white${toggled ? ' scale-100 hover:scale-110' : ' scale-0'}`} />
		</div>
	)
}
...
const Collection = ({
	...
	return (
		<div
			className={`relative  transition-colors bg-contain  border-th-border bg-th-post  shadow-md  select-none ${
		...
		>
			...
			/>
			<div className={`flex flex-col my-2 ${bannerMode ? ' md:mx-16 ' : ' mx-2'}`}>
				<div className={`flex flex-row ${bannerMode ? ' mt-[3.75rem]' : ' mt-6'}`}>
					<div
						className={`z-20 flex-none  rounded-full bg-th-post${
					...
					<div
						className={`z-10 flex flex-row items-baseline gap-2 p-1 pr-4 pl-11 -ml-9 rounded-tr-md pl-auto bg-th-post ${
					...
					>
						...
						{over_18 && <span className='text-xs  text-th-red text-color pb-0.5'>NSFW</span>}
					</div>
				</div>
				<div className={`z-30 flex flex-row pl-5  ${bannerMode ? ' -mt-10 ml-[5.5rem] py-1' : ' -mt-7 ml-14'}`}>
					<div
					...
					>
						<span className='my-auto text-xs opacity-70'>A collection of {subreddits.length} subreddits</span>
						...
						<div className='flex items-center justify-center w-10 h-8 ml-auto border border-transparent rounded-md group-hover:bg-th-highlight group-hover:border-th-borderHighlight '>
							<BsChevronDown className={`${expand ? '-rotate-180 ' : 'rotate-0 '}transform transition duration-200`} />
						</div>
					</div>
					<div className='ml-2 ' />
					<CollectionOptions
						subArray={selected?.filter((s) => subreddits.find((sub) => sub?.toUpperCase() === s?.toUpperCase()))}
						currMulti={name}
						isOwner={isOwner}
					/>
					{!bannerMode && (
							...
							>
								<div className='flex flex-row items-center flex-none ml-auto'>
									...
									<div className='flex-none w-5 h-5 mx-2'>
										<SubButton sub={sub} miniMode={true} userMode={sub?.substring(0, 2) === 'u_'} />
									</div>
								</div>
								<div
								...
								>
									<div className='flex-none w-5 h-5 mr-1'>
										<CheckBox toggled={selected.find((s) => s?.toUpperCase() === sub?.toUpperCase())} />
									</div>
