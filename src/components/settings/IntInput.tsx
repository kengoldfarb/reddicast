...
const IntInput = ({ label, subtext, setting, mini = false, rounded = true, styles = '' }: IntInputProps) => {
	...
	useEffect(() => {
		if (setting === 'fastRefreshInterval' || setting === 'slowRefreshInterval') {
		...
		>
			{mini ? (
				...
				<label className='flex flex-col gap-0.5 hover:cursor-pointer'>
					...
					{inputSubtext && <span className='mr-2 text-xs opacity-70'>{inputSubtext}</span>}
				</label>
			)}
			...
			<div className={`flex flex-col items-end justify-center min-h-full ${mini ? '' : ' my-3  '}`}>
				<div className='relative flex justify-end flex-grow w-20 h-full px-1 text-sm text-right border rounded-md outline-none ring-0 border-th-border bg-th-highlight focus:border-th-borderHighlight'>
					...
					<input
						ref={inputRef}
						className={'bg-transparent outline-none w-full h-full flex-grow text-right'}
						disabled={!context.ready}
						type='text'
						onKeyPress={(event) => {
							if (!/[0-9]/.test(event.key) || +`${inputValue}${event.key}` > MAX || disabled) {
								if (!disabled) {
