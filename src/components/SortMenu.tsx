...
const SortMenu = ({ hide = false, showDropDownIcon = true }) => {
	...
	const updateSort = (e, s) => {
		...
		if (s !== 'top') {
			if (router.query?.sort) {
				...
				let path = router.asPath.replace(`sort=${router.query.sort}`, `sort=${s}`)
				if (router.query?.t) {
				...
				if (isUser) {
					...
					router.push(
						`/${isUser ? 'u' : 'r'}/${router.query?.slug?.[0] ?? 'hot'}/${s}${
							router?.query?.m?.length > 0 ? `?m=${encodeURI(router?.query?.m?.toString())}` : ''
						}`
					)
				}
			} else {
	...
	const updateRange = (e, r, s = 'top') => {
		...
		if (router?.query?.sort) {
			...
			router.push(
				`/r/${router?.query?.slug?.[0]}/search?sort=${s}&t=${r}&q=${encodeURIComponent(router?.query?.q?.toString())}`
			)
		} else if (isUserMulti) {
			...
			if (isUser) {
				...
				router.push(
					`/${isUser ? 'u' : 'r'}/${router.query?.slug?.[0] ?? 'hot'}/top/?t=${encodeURI(r)}${
						router?.query?.m?.length > 0 ? `&m=${encodeURI(router?.query?.m?.toString())}` : ''
					}`
				)
			}
						...
						>
							...
							>
								<div className='py-1'>
											...
											{({ active }) => (
												...
												>
													<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
														...
														<span className={sort === 'best' ? ' font-bold bg-th-highlight ' : ''}>Best</span>
													</div>
												</div>
											)}
										</Menu.Item>
										...
										{({ active }) => (
											...
											>
												<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
													...
													<span className={sort === 'hot' ? ' font-bold ' : ''}>Hot</span>
												</div>
											</div>
										)}
									</Menu.Item>
											...
											<div className='group'>
												...
												>
													<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
														...
														<span className={sort === 'new' ? ' font-bold ' : ''}>New</span>
													</div>
												</div>
											</div>
										)}
											...
											<div className='group'>
												...
												>
													<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
														...
														<span className={sort === 'top' ? ' font-bold ' : ''}>Top</span>
													</div>
												</button>
												<ul
									...
									{/* Relevance */}
									{(isSubFlair || router.route === '/search' || router.query.q) && (
										<Menu.Item disabled={!(isSubFlair || router.route === '/search' || router.query.q)}>
											{({ active }) => (
												<div className='group'>
													<button
														className={classNames(
													...
													>
														<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
															...
															<span className={sort === 'relevance' ? ' font-bold ' : ''}>Relevance</span>
														</div>
													</button>
													{true && (
														<ul
									...
									{(router.route === '/search' || router.query.q) && (
										<Menu.Item disabled={!(router.route === '/search' || router.query.q)}>
											{({ active }) => (
												<div className='group'>
													<button
														className={classNames(
													...
													>
														<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
															...
															<span className={sort === 'comments' ? ' font-bold ' : ''}>Comments</span>
														</div>
													</button>
													{true && (
														<ul
									...
									{/* Rising */}
									{!isUser && !isSubFlair && router.route !== '/search' && !router?.query?.q && (
										<Menu.Item disabled={!(!isUser && !isSubFlair && router.route !== '/search' && !router?.query?.q)}>
											{({ active }) => (
												<div
												...
												>
													<div className='flex flex-row items-center justify-between h-10 cursor-pointer'>
														<IoMdTrendingUp className='flex-none w-5 h-5' />{' '}
														<span className={sort === 'rising' ? ' font-bold' : ''}>Rising</span>
													</div>
												</div>
											)}
										</Menu.Item>
