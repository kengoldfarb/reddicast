...
const Awardings = ({ all_awardings, truncate = true, styles = '' }) => {
	...
	if (all_awardings?.length > 0 && context.showAwardings)
		return (
			<>
				{all_awardings
					?.sort((a, b) => (a?.coin_price > b?.coin_price ? -1 : a?.coin_price < b?.coin_price ? 1 : 0))
					?.slice(0, truncate ? MAX_DISPLAY : Infinity)
					.map((a, i) => {
						return (
							<div key={a?.icon_url ?? i} className={styles} title={`${a?.name} (${a?.count})`}>
								<Image
