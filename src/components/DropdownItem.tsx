...
const DropdownItem = ({
	...
	const Line = (
		<div
			className={`flex flex-row items-center text-sm text-center cursor-pointer group ${className}`}
		...
		>
			...
			<h1 className="ml-2 truncate">
				{sub?.data?.name}
				{/* {isMulti ? sub.data?.name : sub.data?.display_name_prefixed ?? sub.data?.display_name?.replace('u_', '')} */}
